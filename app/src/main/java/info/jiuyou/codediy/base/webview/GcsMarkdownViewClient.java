...
public class GcsMarkdownViewClient extends WebViewClient {
    ...
    public void onLoadResource(WebView view, final String url) {
        ...
        } else if (UrlUtil.isGifSuffix(url)) {
    ...
    private WebResourceResponse getWebResourceResponse(String url) {
        try {
            // 如果是图片且本地有缓存
            if (UrlUtil.isImageSuffix(url) || UrlUtil.isGifSuffix(url)) {
                FileInputStream inputStream = mCache.getStream(url);
                if (null != inputStream) {
                    return new WebResourceResponse(UrlUtil.getMimeType(url), "base64", inputStream);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
